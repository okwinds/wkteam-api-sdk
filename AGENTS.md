...
本项目 `AGENTS.md` 用于把协作方式固化为“宪法”：**Spec-Driven + TDD + 文档可追溯 + 可复现交付**。
本项目是基于 `https://wkteam.cn/api-wen-dang2/` 的离线文档，交付的一套 **工程可用** 的 SDK。

使用方式：
1) 所有变更先写 spec/文档，再动手实现
2) 所有实现必须可回归验证（至少离线单测）
3) 新增文档时同步维护 `DOCS_INDEX.md`
